...
async function main(): Promise<void> {
  ...
  if (before.kind !== 'tournament') {
    log(`FATAL: ${competitionId} is a "${before.kind}" season; only a tournament holds a pool.`);
    db.close();
    process.exit(1);
  }
  ...
  log(`adding pool    ${fmt(poolWei)}`);
  log(`adding jackpot ${fmt(jackpotWei)}`);
  log('');
