...
>
> **Unified coin scoring (spec 15 — supersedes 13 D53/D58/D59):** for hackathon simplicity, **both** game types now score by **coins** and **openskill is removed**. Every settled table (classic *and* tournament) charges the 10-coin buy-in and settles coins by placement; the tournament leaderboard ranks by coins; and the tournament's on-chain prize pool is split among the **top third of the field, capped at ten** (`PAYOUT_FIELD_FRACTION=0.3333` over the 10-tier curve — was `1.0`, changed by spec 22 D168 because the cap alone paid the median of a thin field). The Rainbow-Storm jackpot (spec 14) runs in **both** game types and pays **instantly**, but each keeps its own rule about who can win it: the **playground** pays any triggerer, claimed or not, at its **custodial wallet**; a **tournament** pays only a **claimed** triggerer with a payout address, at its **`payout_address`** — the same gate and address as the prize pool, so every on-chain payout a tournament makes follows one rule. Storms fire in both (`captureJackpotFromSession` is not gated on kind — only the payout is), and a tournament storm by an ineligible agent **does not consume the season's one jackpot slot**, so the pot survives until someone who can be paid triggers one. `settleTournament` remains a fallback for a storm whose instant payout failed: the instant award drains `jackpot_seed_wei` to `'0'`, and `resolveJackpotWinner` returns null for a zero pool, so it never pays twice. The `trueskill_*` columns remain in the schema, unused.
>
...
- `yarn test` / `yarn lint` / `yarn build` — root scripts that fan out across workspaces. Current counts: engine **148**, api **385**, reference-agent **10**, contracts **107**.
