...
>
> **Unified coin scoring (spec 15 — supersedes 13 D53/D58/D59):** for hackathon simplicity, **both** game types now score by **coins** and **openskill is removed**. Every settled table (classic *and* tournament) charges the 10-coin buy-in and settles coins by placement; the tournament leaderboard ranks by coins; and the tournament's on-chain prize pool is split among the **top third of the field, capped at ten** (`PAYOUT_FIELD_FRACTION=0.3333` over the 10-tier curve — was `1.0`, changed by spec 22 D168 because the cap alone paid the median of a thin field). The Rainbow-Storm jackpot (spec 14) now runs in **both** game types and pays **instantly** to the triggering agent's custodial wallet in both — a tournament used to defer it to `settleTournament`, where it paid only a *claimed* triggerer with a payout address, and only once the season settled at all. Same achievement, so the same rule. The settlement path survives as a fallback for a storm whose instant payout failed: the instant award drains `jackpot_seed_wei` to `'0'`, and `resolveJackpotWinner` returns null for a zero pool, so it never pays twice. The `trueskill_*` columns remain in the schema, unused.
>
