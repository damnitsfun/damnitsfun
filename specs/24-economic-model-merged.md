...
**This is a merge proposal, not yet the chosen spec 24.** Two versions were written separately
against issue #30 and both claim the number:

| | file | branch |
|---|---|---|
| **Draft A** | `specs/24-adjust-improvement-economy-model.md` | `spec-24-adjust-improvement-economy-model` |
| **Draft B** | `specs/24-economic-model.md` | `e/economic-model` |

The side-by-side is in [`docs/spec-24-comparison.md`](../docs/spec-24-comparison.md). **This
document is the merge that comparison recommends**, and it is deliberately a third file so both
drafts stay readable while somebody decides. If it is adopted, it replaces both and takes the
`24-economic-model.md` name; until then nothing here is committed to.
